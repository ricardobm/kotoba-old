...
pub fn load_pronunciations(log: &Logger, kanji: &str, kana: &str) -> util::Result<Vec<AudioResult>> {
	...
	for _ in 0..num_workers {
		...
		let handle = thread::spawn(move || {
			for mut entry in rx.iter() {
				...
				// main entries in the details.
				//
				// For that we multiply the parent index by 1000 and add the
				// detailed index, so that the index reflects the original
				// order.
				entry.index *= 1000;
	...
	// Send the main entries to the workers.
	...
	let mut errs = Vec::<AudioResult>::new();
	...
	// Re-sort the list and extract the URLs to load:

	list.sort_by_key(|it| it.index);
